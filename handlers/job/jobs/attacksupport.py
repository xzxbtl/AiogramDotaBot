...
class AttackSupport:
    ...
    async def take_award_from_attack_support(self, query):
        ...
        but1 = InlineKeyboardButton(text="Начать ✔️", callback_data="start_attack_support")
        ...
        try:
            ...
            await logger.error(f"attacksupport.py - Произошла ошибка {err}")

    async def start_attack_support(self, query):
        user_id = query.from_user.id
        ...
        if job_name is None and status_job == "finished":
            ...
            builder = InlineKeyboardBuilder()
            but1 = InlineKeyboardButton(text="Получить ✔️", callback_data="take_award_from_destroy_ward")
            but2 = InlineKeyboardButton(text="⬅️ Назад", callback_data="back_from_jobs_all")
            builder.row(but2, but1, width=2)
            await bot.edit_message_reply_markup(message_id=query.message.message_id,
                                                chat_id=query.message.chat.id,
...
job_methods_attack_support = AttackSupport("Attack Support")


def register_handlers(dp):
    @dp.callback_query(lambda query: query.data == "attack_enemy_support")
    async def attack_support(query: CallbackQuery):
        user_id = query.from_user.id
        try:
            emoji = await TakeInfo.take_hidden_new_value(query)
            if emoji is None:
            ...
            builder = InlineKeyboardBuilder()
            job_name, status_job, take_award = await TakeInfo.take_status_job(user_id)
            if status_job == "processing" and job_name == "Attack Support":
                but1 = InlineKeyboardButton(text="Получить ✔️", callback_data="take_award_from_attack_support")
            else:
                but1 = InlineKeyboardButton(text="Начать ✔️", callback_data="start_attack_support")
            but2 = InlineKeyboardButton(text="⬅ Назад", callback_data="back_from_jobs_all")
            builder.row(but2, but1, width=2)
            photo = FSInputFile(photo_path["attacksupport"])
            await bot.delete_message(chat_id=query.message.chat.id,
                                     message_id=query.message.message_id)
            await bot.send_photo(chat_id=query.message.chat.id,
                                 photo=photo,
                                 caption=f"Вы попали на работу по *Напасть на саппорта* \n Риск этой работы "
                                         f"составляет *70%*"
                                         f"\n За каждый час вы получаете +1700 {emoji} \n Время работы - 2 часа \n "
                                         f"Желаем вам удачи",
                                 parse_mode=ParseMode.MARKDOWN,
                                 reply_markup=builder.as_markup())
        except Exception as err:
            await logger.error(f"attacksupport.py - Ошибка в сообщении {err}")

    @dp.callback_query(lambda query: query.data == "start_attack_support")
    async def start_farms_jungle(query: CallbackQuery):
        await job_methods_attack_support.start_attack_support(query)

    @dp.callback_query(lambda query: query.data == "take_award_from_attack_support")
    async def take_award_from_jungle_job(query: CallbackQuery):
        await job_methods_attack_support.take_award_from_attack_support(query)
