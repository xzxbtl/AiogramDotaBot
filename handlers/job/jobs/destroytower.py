...
class DestroyTower:
    ...
    async def take_award_from_destroy_tower(self, query):
        ...
        but1 = InlineKeyboardButton(text="Начать ✔️", callback_data="start_destroy_tower")
        ...
        try:
            ...
            await logger.error(f"destroytower.py - Произошла ошибка {err}")

    async def start_destroy_tower(self, query):
        user_id = query.from_user.id
        ...
        if job_name is None and status_job == "finished":
            ...
            builder = InlineKeyboardBuilder()
            but1 = InlineKeyboardButton(text="Получить ✔️", callback_data="take_award_from_destroy_tower")
            but2 = InlineKeyboardButton(text="⬅️ Назад", callback_data="back_from_jobs_all")
            builder.row(but2, but1, width=2)
            await bot.edit_message_reply_markup(message_id=query.message.message_id,
                                                chat_id=query.message.chat.id,
...
job_methods_destroy_tower = DestroyTower("Destroy Tower")


def register_handlers(dp):
    @dp.callback_query(lambda query: query.data == "destroy_tower_menu")
    async def destroy_tower(query: CallbackQuery):
        user_id = query.from_user.id
        try:
            emoji = await TakeInfo.take_hidden_new_value(query)
            if emoji is None:
            ...
            builder = InlineKeyboardBuilder()
            job_name, status_job, take_award = await TakeInfo.take_status_job(user_id)
            if status_job == "processing" and job_name == "Destroy Tower":
                but1 = InlineKeyboardButton(text="Получить ✔️", callback_data="take_award_from_destroy_tower")
            else:
                but1 = InlineKeyboardButton(text="Начать ✔️", callback_data="start_destroy_tower")
            but2 = InlineKeyboardButton(text="⬅ Назад", callback_data="back_from_jobs_all")
            builder.row(but2, but1, width=2)
            photo = FSInputFile(photo_path["destroytower"])
            await bot.delete_message(chat_id=query.message.chat.id,
                                     message_id=query.message.message_id)
            await bot.send_photo(chat_id=query.message.chat.id,
                                 photo=photo,
                                 caption=f"Вы попали на работу по *Cломать вражескую башню* \n Риск этой работы "
                                         f"составляет *80%*"
                                         f"\n За каждый час вы получаете +4200 {emoji} \n Время работы - 2 часа \n "
                                         f"Желаем вам удачи",
    ...
    @dp.callback_query(lambda query: query.data == "start_destroy_tower")
    async def start_farms_jungle(query: CallbackQuery):
        await job_methods_destroy_tower.start_destroy_tower(query)

    @dp.callback_query(lambda query: query.data == "take_award_from_destroy_tower")
    async def take_award_from_jungle_job(query: CallbackQuery):
        await job_methods_destroy_tower.take_award_from_destroy_tower(query)
