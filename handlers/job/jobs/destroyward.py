...
class DestroyWard:
    ...
    async def take_award_from_destroy_ward(self, query):
        ...
        but1 = InlineKeyboardButton(text="Начать ✔️", callback_data="start_destroy_ward")
        ...
        try:
            ...
            if job_name == self.set_job_name and status_job == "processing" and not take_award:
                if time_difference.total_seconds() < 10800:
                    remaining_time_seconds = 10800 - time_difference.total_seconds()
            ...
            await logger.error(f"destroyward.py - Произошла ошибка {err}")

    async def start_destroy_ward(self, query):
        user_id = query.from_user.id
...
job_methods_destroy_ward = DestroyWard("Destroy Ward")


def register_handlers(dp):
    @dp.callback_query(lambda query: query.data == "destroy_enemy_ward_menu")
    async def destroy_ward(query: CallbackQuery):
        user_id = query.from_user.id
        try:
            emoji = await TakeInfo.take_hidden_new_value(query)
            if emoji is None:
            ...
            builder = InlineKeyboardBuilder()
            job_name, status_job, take_award = await TakeInfo.take_status_job(user_id)
            if status_job == "processing" and job_name == "Destroy Ward":
                but1 = InlineKeyboardButton(text="Получить ✔️", callback_data="take_award_from_destroy_ward")
            else:
                but1 = InlineKeyboardButton(text="Начать ✔️", callback_data="start_destroy_ward")
            but2 = InlineKeyboardButton(text="⬅ Назад", callback_data="back_from_jobs_all")
            builder.row(but2, but1, width=2)
            photo = FSInputFile(photo_path["destroyward"])
            await bot.delete_message(chat_id=query.message.chat.id,
                                     message_id=query.message.message_id)
            await bot.send_photo(chat_id=query.message.chat.id,
                                 photo=photo,
                                 caption=f"Вы попали на работу по *Сломать вражеский вард* \n Риск этой работы "
                                         f"составляет *40%*"
                                         f"\n За каждый час вы получаете +700 {emoji} \n Время работы - 3 часа \n "
                                         f"Желаем вам удачи",
                                 parse_mode=ParseMode.MARKDOWN,
                                 reply_markup=builder.as_markup())
        except Exception as err:
            await logger.error(f"destroytower.py - Ошибка в сообщении {err}")

    @dp.callback_query(lambda query: query.data == "start_destroy_ward")
    async def start_farms_jungle(query: CallbackQuery):
        await job_methods_destroy_ward.start_destroy_ward(query)

    @dp.callback_query(lambda query: query.data == "take_award_from_destroy_ward")
    async def take_award_from_jungle_job(query: CallbackQuery):
        await job_methods_destroy_ward.take_award_from_destroy_ward(query)
